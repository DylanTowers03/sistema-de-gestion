...
interface FormModalProps {
  ...
  onClose: () => void;
  mode: "create" | "update";
  onSubmit: (data: CategoriasFormData, id?: string) => Promise<void>;
}

export function FormModalCategorias({
  isOpen,
  onClose,
  mode,
  onSubmit,
  ...
  useEffect(() => {
    form.reset({
      nombreCategoria: selectedCategorias?.nombreCategoria || "",
    });
  }, [selectedCategorias]);

  const form = useForm<CategoriasFormData>({
  ...
  const handleSubmit = async (data: CategoriasFormData) => {
    setIsSubmitting(true);
    try {
      await onSubmit(data, selectedCategorias?.id);
      form.reset();
        ...
        <DialogHeader>
          <div className="flex items-center space-x-2">
            ...
            <div>
              ...
              <DialogDescription>
                {mode === "create"
                  ? `Completa la información para crear un nuevo Categorias`
                  : `Modifica la información del Categorias seleccionado`}
              </DialogDescription>
            </div>
          </div>
          <Badge
        ...
        {mode === "update" && (
          <div className="space-y-4">
            <div className="relative">
              ...
              <Input
                placeholder={`Buscar por nombre o ID del Categorias...`}
                value={searchTerm}
                ...
                <div className="p-2 space-y-2">
                      ...
                      >
                        <Card
                          className={`cursor-pointer transition-all duration-200 hover:shadow-md border-none shadow-none ${
                            selectedCategorias?.id === Categorias.id
                              ? "ring-2 ring-blue-500 bg-blue-50 dark:bg-red-900/10"
                              : "hover:bg-muted/50"
                          }`}
                          onClick={() => setSelectedCategorias(Categorias)}
                        >
                          <CardContent>
                            <div className="flex items-center justify-between">
                  ...
                  {categorias.length === 0 && (
                    <div className="text-center py-8 text-muted-foreground">
                      ...
                      <p className="text-sm">No se encontraron categorias</p>
                    </div>
                  )}
                </div>
              </ScrollArea>
          ...
          >
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Nombre del Categorias</FormLabel>
                    <FormControl>
                      <Input
                        placeholder={`${"Nombre de las Categoria"}`}
                        {...field}
                      />
                    </FormControl>
                    <FormMessage />
